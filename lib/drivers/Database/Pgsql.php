...
class AetherDatabasePgsqlDriver extends AetherDatabaseDriver {
	...
	public function connect() {
		...
		$connection_string = $host.' dbname=\''.$database.'\' user=\''
            .$user.'\' password=\''.$pass.'\'';
	...
	public function query($sql) {
		...
		if ($this->db_config['cache'] AND 
			...
			if (!isset(self::$query_cache[$hash])) {
				...
				self::$query_cache[$hash] = new AetherPgsqlResult(
                    pg_query($this->link, $sql), $this->link, 
		...
		return new AetherPgsqlResult(pg_query($this->link, $sql), 
	...
	public function list_fields($table) {
		...
		if (!isset($tables[$table]))
			throw new AetherDatabaseException('database.table_not_found', $table);
...
} // End AetherDatabasePgsqlDriver Class
	...
	public function __construct($result, $link, $object = TRUE, $sql) {
		...
		if (is_resource($result)) {
			...
			if (preg_match('/^(?:delete|insert|replace|update)\b/iD', trim($sql), $matches)) {
				$this->insert_id  = (strtolower($matches[0]) == 'insert') ? 
			...
			throw new AetherDatabaseException('database.error', pg_last_error().' - '.$sql);
		}
